...
//`endif
  ...
  always @(posedge clk_i or posedge rst_i)
    if (rst_i) load_REG <= 0;
    else if (wb_we & (adr_i[8-1:0] == load_REG_OFFSET)) load_REG <= dat_i[32-1:0];

  reg [0:0] control_REG;
  assign WDTEN = control_REG;
  always @(posedge clk_i or posedge rst_i)
    if (rst_i) control_REG <= 0;
    else if (wb_we & (adr_i[8-1:0] == control_REG_OFFSET)) control_REG <= dat_i[1-1:0];
  ...
  always @(posedge clk_i or posedge rst_i)
    if (rst_i) IM_REG <= 0;
    else if (wb_we & (adr_i[8-1:0] == IM_REG_OFFSET)) IM_REG <= dat_i[1-1:0];
  always @(posedge clk_i or posedge rst_i)
    if (rst_i) IC_REG <= 1'b0;
    else if (wb_we & (adr_i[8-1:0] == IC_REG_OFFSET)) IC_REG <= dat_i[1-1:0];
    else IC_REG <= 1'd0;
  ...
  assign	dat_o = 
			...
			(adr_i[8-1:0] == load_REG_OFFSET)	? load_REG :
			(adr_i[8-1:0] == control_REG_OFFSET)	? control_REG :
			(adr_i[8-1:0] == IM_REG_OFFSET)	? IM_REG :
			(adr_i[8-1:0] == MIS_REG_OFFSET)	? MIS_REG :
			(adr_i[8-1:0] == RIS_REG_OFFSET)	? RIS_REG :
			(adr_i[8-1:0] == IC_REG_OFFSET)	? IC_REG :
			32'hDEADBEEF;
